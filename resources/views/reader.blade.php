...
<div class="det">
	...
	<div class="row">
    	...
    	<!-- BEGIN PRODUCTS -->
  		<div class="site-heading"><H4>READERS</H4></div>
    </div>
  ...
  @foreach($readers as $reader)
       <div class="col-md-3 col-sm-6">
        <span class="thumbnail">
            <img src="/img/reader/{{$reader->img}}" alt="...">
            <h4><b>{{$reader->title}}</b></h4>
            <div class="ratings">
                    ...
                    ({{$reader->review}} reviews)
                </div>
           
            <hr class="line">
            <div class="row">
              <div class="col-md-6 col-sm-6">
                <p class="price"><b><h4>Rs {{$reader->price}}.00</h4></b></p>
              </div>
             <div class="btn-ground text-center">
                   
                    <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#product_view{{$reader->id}}"><i class="fa fa-search"></i> Quick View</button>
                </div>
            </div>
        </span>
      </div>

      <div class="modal fade product_view" id="product_view{{$reader->id}}">
      <div class="modal-dialog">
      <div class="modal-content">
         <div class="modal-header">
                <a href="#" data-dismiss="modal" class="class pull-right"><span class="glyphicon glyphicon-remove"></span></a>
                <h3 class="modal-title">{{$reader->title}}</h3>
            </div>
             <div class="modal-body">
              <div class="row">
                    <div class="col-md-6 product_img">
                        <img src="/img/reader/{{$reader->img}}" class="img-responsive">
                    </div>
                     <div class="col-md-6 product_content">
                     <h4>Product Id: <span>RE{{$reader->id}}</span></h4>
                        <div class="rating">
                            <span class="glyphicon glyphicon-star"></span>
                            <span class="glyphicon glyphicon-star"></span>
                            <span class="glyphicon glyphicon-star"></span>
                        ...
                        <p>{{$reader->discription}}</p>
                        <ul class="a"><h6><b>
                          <li>brand: {{$reader->brand}}</li>
                          <li>frame meterial: {{$reader->frame_metrial}}</li>
                          <li>frame shap: {{$reader->frame_shap}} </li>
                          <li>frame style: {{$reader->frame_style}} </li>
                          <li>frame type: {{$reader->frame_type}} </li>
                          <li>frame color: {{$reader->frame_color}} </li>
                          <!-- <li>lense type: {{$reader->lense_type}} </li> -->
                          </b>
                          </h6>
                        </ul>
                        <h3 class="cost"><b><h4>Rs {{$reader->price}}.00</h4></b> </h3>
<div class="row">
                        ...
                        {{csrf_field()}}
                        <input type="hidden" name="item" value="reader"/>
                        <input type="hidden" name="item_id" value="RE{{$reader->id}}"/>
                        <input type="hidden" name="price" value="Rs{{$reader->price}}"/>
                        <input type="hidden" name="title" value="{{$reader->title}}"/>
